...
def validate_opera_pms(hotel_id: int, feature_flags: dict[str, bool] | None = None) -> Any:
    ...
    with ValidationRun(
        ...
        step_config = ValidationStep(name="load_config", passed=False, timestamp=datetime.now())
        ...
        step_read = ValidationStep(name="test_read_access", passed=False, timestamp=datetime.now())
        ...
        step_write = ValidationStep(
            name="test_write_access", passed=False, timestamp=datetime.now()
        )
