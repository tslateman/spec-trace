...
class Requirement(MP_Node):
    ...
    structure_completeness = models.FloatField(
        default=0.0, help_text="Percentage of structured fields populated (0.0-1.0)"
    )

    # Source tracking
    source_file = models.CharField(max_length=500, help_text="Relative path to source spec file")
...
class TestRun(models.Model):
    ...
    imported_at = models.DateTimeField(
        auto_now_add=True, help_text="When the test results were imported"
    )
    source_file = models.CharField(max_length=500, help_text="Path to JUnit XML file")
    # CI metadata
    git_sha = models.CharField(
        max_length=40, blank=True, help_text="Git commit SHA for this test run"
    )
    git_branch = models.CharField(
        max_length=200, blank=True, help_text="Git branch for this test run"
    )
    ...
    workflow_name = models.CharField(
        max_length=200, blank=True, help_text="GitHub Actions workflow name"
    )
    workflow_run_id = models.BigIntegerField(
        null=True, blank=True, db_index=True, help_text="GitHub Actions workflow run ID"
    )
    ...
    started_at = models.DateTimeField(null=True, blank=True, help_text="When the test run started")
    finished_at = models.DateTimeField(
        null=True, blank=True, help_text="When the test run finished"
    )
...
class TestResult(models.Model):
    ...
    classname = models.CharField(
        max_length=300, blank=True, help_text="Test class name from JUnit XML"
    )
    name = models.CharField(max_length=200, help_text="Test function/method name")
    time = models.FloatField(default=0.0, help_text="Test execution time in seconds")
    status = models.CharField(max_length=20, choices=Status.choices, help_text="Test result status")
...
class InAppValidationRun(models.Model):
    ...
    imported_at = models.DateTimeField(
        auto_now_add=True, help_text="When the validation results were imported"
    )
    source = models.CharField(
        max_length=500, help_text="Source file or system that provided the results"
    )
...
class InAppValidationResult(models.Model):
    ...
    validation_run = models.ForeignKey(
        InAppValidationRun, on_delete=models.CASCADE, related_name="results"
    )
    validation = models.ForeignKey(
        InAppValidation, on_delete=models.CASCADE, related_name="results"
    )
    status = models.CharField(
        max_length=20, choices=InAppValidationStatus.choices, help_text="Result status"
    )
...
class VerificationFlow(models.Model):
    ...
    steps = models.JSONField(default=list, help_text="Ordered list of step definitions")
    version = models.PositiveIntegerField(default=1, help_text="Flow version number")
    synced_at = models.DateTimeField(null=True, blank=True, help_text="When last synced from code")
...
class VerificationFlowRun(models.Model):
    ...
    context = models.JSONField(default=dict, help_text="Execution context and state")
    source = models.CharField(
        max_length=20,
...
class VerificationFlowStep(models.Model):
    ...
    step_order = models.PositiveIntegerField(help_text="Order of execution (0-indexed)")
    name = models.CharField(max_length=100, help_text="Step name from flow definition")
    passed = models.BooleanField(help_text="Whether the step passed")
    details = models.TextField(blank=True, help_text="Human-readable success details")
    error_message = models.TextField(blank=True, help_text="Error description when step fails")
    response_status = models.IntegerField(
        null=True, blank=True, help_text="HTTP status code if applicable"
    )
    response_body = models.TextField(
        blank=True, help_text="Sanitized response content for debugging"
    )
    started_at = models.DateTimeField(help_text="When step execution started")
    completed_at = models.DateTimeField(help_text="When step execution completed")
...
class TestRequirementLink(models.Model):
    ...
    last_run_at = models.DateTimeField(
        null=True, blank=True, help_text="When this test was last run"
    )
...
class ConflictLog(models.Model):
    ...
    resolved = models.BooleanField(
        default=False, help_text="Whether this conflict has been reviewed and resolved"
    )
    resolved_at = models.DateTimeField(
        null=True, blank=True, help_text="When the conflict was marked as resolved"
    )
    resolution_notes = models.TextField(
        blank=True, help_text="Notes on how the conflict was resolved"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        ...
        return (
            f"{self.requirement_a.external_id} ↔ {self.requirement_b.external_id}"
            f" ({self.pattern}) {status}"
        )
...
class SLO(models.Model):
    ...
    display_name = models.CharField(
        max_length=200, blank=True, help_text="Human-readable display name"
    )
    ...
    service = models.CharField(
        max_length=200, blank=True, help_text="Service name from OpenSLO spec"
    )
    ...
    time_window = models.CharField(
        max_length=50, blank=True, help_text="Time window for SLO (e.g., '30d', '7d')"
    )
    ...
    source_file = models.CharField(
        max_length=500, blank=True, help_text="Path to OpenSLO YAML source file"
    )
...
class AgentSprint(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="Sprint name (e.g., 'Auth Flow v2')")
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether new tasks can be added to this sprint"
    )
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the sprint was marked complete"
    )
...
class Agent(models.Model):
    ...
    # Status
    is_active = models.BooleanField(
        default=True, help_text="Whether agent is active and can claim tasks"
    )
    last_heartbeat = models.DateTimeField(
        null=True, blank=True, help_text="When agent last reported being alive"
    )
...
class AgentTask(models.Model):
    ...
    claimed_at = models.DateTimeField(null=True, blank=True, help_text="When the task was claimed")
    lease_expires = models.DateTimeField(
        null=True, blank=True, help_text="Task returns to UNCLAIMED if lease expires"
    )
    ...
    scope_out = models.JSONField(
        default=list, help_text="What is NOT in scope (explicit exclusions)"
    )
    ...
    branch_name = models.CharField(max_length=100, blank=True, help_text="Git branch for this task")
    commit_sha = models.CharField(
        max_length=40, blank=True, help_text="Latest commit SHA submitted for review"
    )
    ...
    attempt_count = models.PositiveIntegerField(
        default=0, help_text="Number of times this task has been attempted"
    )
...
AGENT_TASK_STATE_TRANSITIONS = {
    ...
    AgentTaskStatus.READY_FOR_REVIEW: [
        ...
        AgentTaskStatus.CHANGES_REQUESTED,
    ],
    AgentTaskStatus.CHANGES_REQUESTED: [
        AgentTaskStatus.READY_FOR_REVIEW,
...
class AgentTaskHistory(models.Model):
    ...
    from_status = models.CharField(max_length=20, blank=True, help_text="Status before transition")
    to_status = models.CharField(max_length=20, blank=True, help_text="Status after transition")

    # Additional context
    details = models.JSONField(
        default=dict, help_text="Additional context (commit SHA, review feedback, etc.)"
    )
...
class AgentTaskReview(models.Model):
    ...
    decision = models.CharField(
        max_length=20, choices=ReviewDecision.choices, help_text="Review outcome"
    )
...
class WebhookEvent(models.Model):
    ...
    event_type = models.CharField(
        max_length=50, db_index=True, help_text="Event type (e.g., 'workflow_run')"
    )
    action = models.CharField(
        max_length=50, blank=True, help_text="Event action (e.g., 'completed')"
    )

    # Source
    repository = models.CharField(
        max_length=200, db_index=True, help_text="Repository full name (owner/repo)"
    )
    sender = models.CharField(
        max_length=100, blank=True, help_text="GitHub username that triggered the event"
    )
    ...
    received_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="When webhook was received"
    )
