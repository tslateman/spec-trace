...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("junit_xml", type=str, help="Path to pytest JUnit XML file")
        parser.add_argument(
            "--links", type=str, help="Path to extract_links JSON output (optional)"
        )
        parser.add_argument(
            "--no-status-update",
            action="store_true",
        ...
        parser.add_argument(
            "--git-sha", type=str, default="", help="Git commit SHA for this test run"
        )
        parser.add_argument(
            "--git-branch", type=str, default="", help="Git branch for this test run"
        )
        parser.add_argument(
            "--ci-job-url",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if links_path:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Linked {summary['linked_count']} test results to requirements")
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Updated {link_summary['updated_count']} test-requirement links")
        )
        if link_summary["status_changes"]:
            self.stdout.write(
                self.style.WARNING(
                    f"  {len(link_summary['status_changes'])} links changed status"
                    " (flagged for review)"
                )
            )
