    ...
    def _output_text(self, result, check, fix):
        ...
        for code, violations in sorted(by_code.items()):
            ...
            for v in errors:
                ...
                self.stdout.write(self.style.ERROR(f"  {marker} {v.requirement_id}: {v.message}"))

            for v in warnings:
                marker = "⚠" if not v.fixable else ("✓ fixed" if fix else "⚠ (fixable)")
                self.stdout.write(self.style.WARNING(f"  {marker} {v.requirement_id}: {v.message}"))
        ...
        if fix and result.fixed_count > 0:
            self.stdout.write(self.style.SUCCESS(f"Fixed {result.fixed_count} violations"))
