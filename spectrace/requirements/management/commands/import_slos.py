...
class Command(BaseImportCommand):
    ...
    def do_import(self, path: Path, options: dict):
        ...
        for slo in slos:
            ...
            self.stdout.write(
                f"  - {slo['name']}: target={target_str}, linked_requirements={req_count}"
            )
        ...
        # Import to database
        created = import_slos_to_database(slos, clear_existing=options["clear"])

        self.stdout.write(
            self.style.SUCCESS(
                f"Import complete: {created} new SLO(s) created, {len(slos) - created} updated"
            )
        )
