...
def get_validation_runs_data(
    page: int = 1, per_page: int = 25, filters: dict | None = None
) -> dict:
    ...
    if filters.get("requirement"):
        queryset = queryset.filter(
            results__validation__requirement__external_id=filters["requirement"]
        ).distinct()
    ...
    unique_vendors = (
        InAppValidationResult.objects.values("validation__vendor")
        .distinct()
        .exclude(validation__vendor="")
        .count()
    )
...
def build_run_comparison(run_a: InAppValidationRun, run_b: InAppValidationRun) -> dict:
    ...
    results_a = {r.validation_id: r for r in run_a.results.select_related("validation")}
    results_b = {r.validation_id: r for r in run_b.results.select_related("validation")}
    ...
    for val_id in all_validation_ids:
        ...
        if result_a is None:
            ...
            if status_a == status_b:
                ...
                status_a == InAppValidationStatus.FAILURE
                and status_b == InAppValidationStatus.SUCCESS
            ):
                change_type = "improved"
            elif (
                status_a == InAppValidationStatus.SUCCESS
                and status_b == InAppValidationStatus.FAILURE
            ):
    ...
    changes.sort(
        key=lambda c: (
            change_order.get(c["change_type"], 5),
            c["vendor"],
            c["validation_name"],
        )
    )
...
def get_adjacent_runs(run: InAppValidationRun) -> dict:
    ...
    """
    previous_run = (
        InAppValidationRun.objects.filter(imported_at__lt=run.imported_at)
        .order_by("-imported_at")
        .first()
    )

    next_run = (
        InAppValidationRun.objects.filter(imported_at__gt=run.imported_at)
        .order_by("imported_at")
        .first()
    )
...
def get_unique_vendors() -> list:
    ...
    """
    vendors = (
        InAppValidationResult.objects.values_list("validation__vendor", flat=True)
        .distinct()
        .exclude(validation__vendor="")
    )
...
def get_unique_sources() -> list:
    ...
    """
    sources = InAppValidationRun.objects.values_list("source", flat=True).distinct()
